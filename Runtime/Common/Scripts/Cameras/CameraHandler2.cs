...
{
    public class CameraHandler2 : MonoBehaviour, IInputCallBacks
    {
        #region Properties
        ...
        private CinemachineOrbitalFollow _transposer;
        ...
        public float fov { get => (_virtualCamera as CinemachineCamera).Lens.FieldOfView; set { (_virtualCamera as CinemachineCamera).Lens.FieldOfView = value; } }
        ...
        private void Awake()
        {
            ...
            var cam = (_virtualCamera as CinemachineCamera);
            var aim = cam.GetCinemachineComponent(CinemachineCore.Stage.Aim);
            _pov =  aim as CinemachinePanTilt;
            _transposer = cam.GetCinemachineComponent(CinemachineCore.Stage.Body) as CinemachineOrbitalFollow;
            enabled = false;
        }
        ...
        public void ResetCamera()
        {
            if (interestItem)
            {
                ...
                if (_pov)
                {
                    _pov.PanAxis.Value = 0;
                    _pov.TiltAxis.Value = target.eulerAngles.y;
                }
                ...
                {
                    _transposer.TargetOffset.y = _initialYaw;
                    _transposer.TargetOffset.z = _yawRadius;
                    _transposer.HorizontalAxis.Value = target.eulerAngles.y;

                }


                _zoom = _initialZoom = _zoomMin;
                if (_offset) _offset.Offset.z = _zoom;

                if (interestItem.overrideCameraBlend)
                {
                    brain.DefaultBlend = interestItem.cameraBlend;
                }
                else
                {
                    switch (cameraType)
                    {
                        case State.FPS:
                            brain.DefaultBlend = config.fpsCameraBlend;
                            break;
                        case State.LOCKEDTHIRD:
                            brain.DefaultBlend = config.thirdCameraBlend;
                            break;
                        case State.LOCKED:
                            brain.DefaultBlend = config.lockedCameraBlend;
                            break;
                        case State.PAN:
                            brain.DefaultBlend = config.panCameraBlend;
                            break;
                        default:
                            break;
                    }
        ...
        private void SetHorizontalAxis(float value)
        {
            ...
            _xAxis = _initialX + value * config.sesitivity.x;
                    if (_transposer)
                        _transposer.HorizontalAxis.Value = _xAxis;
                    else
            ...
            switch (cameraType)
            {
                ...
                case State.PAN:
                    ...
                    if (_transposer)
                    {
                        ...
                        _transposer.HorizontalAxis.Value = _yAxis;
        ...
        public void OnTouch(float x, float y)
        {
            ...
            switch (cameraType)
            {
                case State.FPS:
                    if (_pov)
                    {
                        _initialX = _pov.TiltAxis.Value;
                        _initialY = _pov.PanAxis.Value;
                    }
                    else
                        Debug.LogError($"[CameraHandler] POV is missing in {name} game object.");
                    break;
                ...
                case State.PAN:
                    if (_transposer)
                    {
                        _initialX = _transposer.HorizontalAxis.Value;
                        _initialY = _transposer.VerticalAxis.Value;
        ...
        public IEnumerator Recenter()
        {
            ...
            if (_pov)
                startY = _pov.TiltAxis.Value;
            else
                if (_transposer)
                startY = _transposer.TargetOffset.y;

            float startZ = _offset ? _offset.Offset.z : 0;
            ...
            while (elapsed < duration)
            {
                ...
                if (_pov)
                {
                    _pov.TiltAxis.Value = Mathf.Lerp(startY, _initialYaw, t);
                }
                else
                if (_transposer)
                {
                    _transposer.TargetOffset.y = Mathf.Lerp(startY, _initialYaw, t);
                }


                if (_offset)
                    _offset.Offset.z = Mathf.Lerp(startZ, _initialZoom, t);
        ...
        public IEnumerator StandBy()
        {
            ...
            while (true)
            {
                ...
                if (_pov)
                {
                    ...
                    _pov.TiltAxis.Value += config.standbyRotationSpeed * Time.deltaTime;
                }
                else
                if (_transposer)
                {
                    _transposer.HorizontalAxis.Value += config.standbyRotationSpeed * Time.deltaTime;
                }
                yield return null;
            }
        }
