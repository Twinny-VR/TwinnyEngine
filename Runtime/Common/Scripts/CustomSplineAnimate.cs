...
namespace Twinny.GamePlay
{
    ...
    /// </summary>
    [AddComponentMenu("Twinny/Custom Spline Animate")]
    [ExecuteInEditMode]
    public class CustomSplineAnimate : SplineComponent
    {
        ...
        public float StartOffsetT => m_StartOffsetT;
        ...
        public void RecalculateAnimationParameters()
        {
            RebuildSplinePath();
        ...
        public static readonly string k_EmptyContainerError = "SplineAnimate does not have a valid SplineContainer set.";
...
#if UNITY_EDITOR
        [ContextMenu("Play no Editor")]
        public void PlayNoEditor()
        {
            if (!EditorApplication.isPlaying)
            {
                Play();
        ...
        public void Update()
        {
            ...
            if (m_breakPoints.Length == 0 || m_currentBreakPoint == null) return;

            if (m_currentBreakPoint.state == BreakPoint.BreakPointState.IDLE)
            {

                int currentKnot = GetCurrentKnot();
                if (currentKnot >= m_currentBreakPoint.position)
                    SetPause();
        ...
        public float GetLoopInterpolation(bool offset)
        {
            var t = 0f;
        ...
        [SerializeField] private BreakPoint[] m_breakPoints;
        private Queue<BreakPoint> m_breakPoinQueue = new Queue<BreakPoint>();
        private BreakPoint m_currentBreakPoint;

        private void Start()
        {
            if (m_breakPoints.Length > 0)
            {
                foreach (var bp in m_breakPoints) m_breakPoinQueue.Enqueue(bp);
                m_currentBreakPoint = m_breakPoinQueue.Dequeue();
            }
        }
        ...
        int GetCurrentKnot()
        {
            ...
            float knotT = m_SplinePath.ConvertIndexUnit(normalizedTime, PathIndexUnit.Normalized, PathIndexUnit.Knot);

            return Mathf.FloorToInt(knotT);
        ...
        async void SetPause()
        {
            ...
            if (await m_currentBreakPoint.Break())
            {
                Resume();
            }
        }
        ...
        public void Resume()
        {
            if (m_currentBreakPoint == null) return;

            m_currentBreakPoint.Resume();

            m_currentBreakPoint = (m_breakPoinQueue.Count > 0) ? m_breakPoinQueue.Dequeue() : null;
