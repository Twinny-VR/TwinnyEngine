...
namespace Twinny
{
    public class AvatarSpawner : TSingleton<AvatarSpawner>
    {
        ...
        */
        ...
        {
            Debug.LogWarning("[AvatarSpawner] OnHMDMounted event. "+ SceneFeatureXR.Instance?.sceneType);

            if (SceneFeatureXR.Instance && SceneFeatureXR.Instance.sceneType == SceneType.VR)
                SpawnAvatar();
        }
